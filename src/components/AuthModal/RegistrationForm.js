...
import {
  Divider, Form, Label, Input, Button, Icon, Message,
...
class RegistrationForm extends React.Component {
	...
	signIn = () => {
	  ...
	  if (this.validateForm()) {
	    firebase.createUser({ email, password }).then((data) => {
...
Password (6 or more characters)
      ...
      <div className="mail-buttons d-flex justify-content-between">
        <Button type="submit" onClick={this.signIn} secondary>
Sign In
        </Button>
        <Button onClick={dialogClose} as={Link} to="/">
Cancel
        </Button>
      </div>
      <Divider />
